...
- *Voice & TTS Speech Sanitization: Markdown syntax (`**`, `#`, `[links]()`) must be cleaned prior to feeding assistant strings to Text-to-Speech (`FlutterTts`), and agent voice readouts should be capped to the first 2 punchy sentences to keep responses conversational.*
- *Cross-Feature Widget Imports: When importing reusable widgets across feature modules in Flutter, verify canonical directory paths (e.g. `features/flights/widgets/` vs `features/flight_search/widgets/`) to prevent URI resolution breaks.*
- *Android Gradle Maven Repository Resolution & IPv6 Timeouts: On macOS/Android builds, Java may try to resolve Kotlin dependencies (`org.jetbrains.kotlin:*`) against `dl.google.com` (Google Maven) over IPv6, causing 2–3 minute network `Read timed out` errors. Always place `mavenCentral()` first with content filtering on `google { content { includeGroupByRegex(...) } }`, and add `-Djava.net.preferIPv4Stack=true` with socket timeouts in `android/gradle.properties`.*
- *Android Speech-to-Text (`speech_to_text`): On Android, `RECORD_AUDIO` permission must be declared in `AndroidManifest.xml`, and on Android 11+ (API 30+), the `<queries>` block must declare `<intent><action android:name="android.speech.RecognitionService"/></intent>` for package visibility to bind to Google Speech Services; otherwise initialization fails with `SpeechToTextNotInitializedException`.*
